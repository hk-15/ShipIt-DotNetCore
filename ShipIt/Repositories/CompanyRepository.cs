...
namespace ShipIt.Repositories
{
    public interface ICompanyRepository
    {
        ...
        IEnumerable<CompanyDataModel> GetAllCompanies(IEnumerable<string> gcps);
    ...
    public class CompanyRepository : RepositoryBase, ICompanyRepository
    {
        ...
        public IEnumerable<CompanyDataModel> GetAllCompanies(IEnumerable<string> gcps)
        {
            string sql =
                String.Format("SELECT gcp_cd, gln_nm, gln_addr_02, gln_addr_03, gln_addr_04, gln_addr_postalcode, gln_addr_city, contact_tel, contact_mail FROM gcp WHERE gcp_cd IN ('{0}')",
                 String.Join("','", gcps));
            //var parameter = new NpgsqlParameter("@p_id", ids);
            string noProductWithIdErrorMessage = string.Format(
                "No company found with id of value {0}",
                gcps.ToString()
            );
            return base.RunGetQuery(
                sql,
                reader => new CompanyDataModel(reader),
